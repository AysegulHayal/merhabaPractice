...
static double bakiye=50000;
  ...
  Para gönderme işleminde istenen iban TR ile baslamali ve toplam 26 karakterli olmali, eger değilse menü ekranina geri donsun.*/
        scan.nextLine();//dummy
        System.out.println("PARA GONDERILECEK IBANI GIRINIZ: ");
        String iban=scan.nextLine().toUpperCase().replace(" ","");
        if (iban.startsWith("TR")&& iban.length()==26){
            System.out.println("GONDERILECEK MIKTAR GIRINIZ: ");
            double miktar = scan.nextDouble();
           if (miktar<=bakiye) {
               bakiye -= miktar;
               bakiyeSorgula();
    ...
    public static void paraCekme(double miktar) {
       // Para çekme ve para gonderme işleminde mevcut bakiyeden buyuk para çekilemez,
        if (miktar<=bakiye){
            ...
            paraCekme(yeniMiktar); // methodun içinden kendisini cagirma "özyinemeli recursive method" denir
        }
    }
